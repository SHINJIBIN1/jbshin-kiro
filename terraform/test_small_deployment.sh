#!/bin/bash
# Script to test the small-scale deployment
...
# Plan the small-scale deployment
print_section "Planning small-scale deployment"
terraform plan -var="deployment_scale=small" -var="auto_scale_enabled=false" -out=small_deployment.tfplan

# Apply if requested
if [ "$APPLY" = true ]; then
  print_section "Applying small-scale deployment"
  terraform apply small_deployment.tfplan
  ...
  EC2_ID=$(terraform output -json | jq -r '.instance_id.value // empty')
  if [ -n "$EC2_ID" ]; then
    echo -e "${GREEN}✓ EC2 instance created successfully: $EC2_ID${NC}"
    
    # Check instance status
    aws ec2 describe-instance-status --instance-ids $EC2_ID --query 'InstanceStatuses[0].InstanceStatus.Status' --output text
  else
    echo -e "${RED}✗ EC2 instance not found in Terraform outputs${NC}"
  fi
  ...
  aws cloudwatch list-metrics --namespace AWS/EC2 --dimensions Name=InstanceId,Value=$EC2_ID --query 'Metrics[0:5]'
  ...
  # Test 1: Check if the deployment scale is correctly set to small
  ...
  if [ "$SCALE" = "small" ]; then
    echo -e "${GREEN}✓ Deployment scale is correctly set to 'small'${NC}"
  else
    echo -e "${RED}✗ Deployment scale is not set to 'small' (current value: $SCALE)${NC}"
  fi
  
  # Test 2: Verify that no load balancer exists (small scale shouldn't have one)
  ALB_COUNT=$(aws elbv2 describe-load-balancers --query 'length(LoadBalancers)' --output text 2>/dev/null || echo "0")
  if [ "$ALB_COUNT" = "0" ]; then
    echo -e "${GREEN}✓ No load balancer exists (as expected for small scale)${NC}"
  else
    echo -e "${RED}✗ Load balancer exists but shouldn't for small scale deployment${NC}"
  fi
  
  # Test 3: Verify that no auto scaling group exists (small scale shouldn't have one)
  ASG_COUNT=$(aws autoscaling describe-auto-scaling-groups --query 'length(AutoScalingGroups)' --output text 2>/dev/null || echo "0")
  if [ "$ASG_COUNT" = "0" ]; then
    echo -e "${GREEN}✓ No auto scaling group exists (as expected for small scale)${NC}"
  else
    echo -e "${RED}✗ Auto scaling group exists but shouldn't for small scale deployment${NC}"
  fi
  ...
  echo -e "${GREEN}Small-scale deployment test completed.${NC}"
  
  # Destroy if requested
  if [ "$DESTROY" = true ]; then
    print_section "Destroying infrastructure"
    terraform destroy -var="deployment_scale=small" -var="auto_scale_enabled=false" -auto-approve
    echo -e "${GREEN}Infrastructure destroyed successfully.${NC}"
  else
    echo -e "${YELLOW}Infrastructure remains deployed. To destroy it, run:${NC}"
    echo "terraform destroy -var=\"deployment_scale=small\" -var=\"auto_scale_enabled=false\""
  fi
else
